...
def validate_authoritative(record_meta, schema, errors, warnings):
    ...
    if status not in ("authoritative_missing", "restricted_withheld"):
        if not opath.exists():
    ...
    tpath = vdir / "text.txt"
    if status in ("authoritative_missing", "restricted_withheld"):
        if tpath.exists():
            errors.append(f"[policy] {rel}: {status} must not contain text.txt")
    else:
        if not tpath.exists():
